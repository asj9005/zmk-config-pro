...
static int split_central_esb_get_available_source_ids(uint8_t *sources) {
    ...
    return CONFIG_ZMK_SPLIT_ESB_PERIPHERAL_COUNT;
}
        ...
        while (ring_buf_size_get(rx_buf) > ESB_MSG_EXTRA_SIZE) {
            ...
            case 0: {
                ...
                if (env.payload.wire_type == ESB_WIRE_EVENT_ZMK) {
                    event_type = env.payload.event.type;
                    if (env.payload.event.type ==
                        ZMK_SPLIT_TRANSPORT_PERIPHERAL_EVENT_TYPE_KEY_POSITION_EVENT) {
                        position = env.payload.event.data.key_position_event.position;
                        pressed = env.payload.event.data.key_position_event.pressed;
                    }
                }
                totem_esb_benchmark_rx(source, env.payload.sequence, gap,
                                       env.payload.source_tick, env.payload.wire_type,
                                       event_type, position, pressed);

                if (!accept || env.payload.wire_type != ESB_WIRE_EVENT_ZMK ||
                ...
                zmk_split_transport_central_peripheral_event_handler(
                    &esb_central, source, env.payload.event);
                break;
            }
            case -EAGAIN:
                break;
