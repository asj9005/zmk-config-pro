...
BLE configure restores the Prospector originals from those backups, so building
ESB and then BLE in the same west workspace does not leave the rollback build
using the ESB-only display code.

These overlays are revision-specific. When changing either upstream SHA,
compare every overlaid file with the new upstream version and re-run all ten
