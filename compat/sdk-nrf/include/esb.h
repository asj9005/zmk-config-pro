...
/** Count queued ACK payloads on one PRX pipe; negative errno on invalid use. */
int esb_get_ack_payload_count(uint8_t pipe);

/**
 * Cancel one PRX pipe's pending ACK payloads without discarding RX packets or
 * changing other pipes. Returns -EBUSY while an ACK is being transmitted.
 * An ACK already sent cannot be revoked; callers must still reject old sessions.
 */
