...
static void esb_timer_handler(nrf_timer_event_t event_type, void *context)
{
	if (event_type == NRF_TIMER_EVENT_COMPARE1 &&
	    nrf_timer_int_enable_check(esb_timer.p_reg, NRF_TIMER_INT_COMPARE1_MASK)) {
...
int esb_get_diagnostics(struct esb_diagnostics *out)
{
	...
	/* Independent scalar observations, not an atomic state snapshot. Do not
	 * lock interrupts, acknowledge events, or capture into a timer CC channel.
	 */
	*out = (struct esb_diagnostics){
		.state = (uint32_t)esb_state,
		.radio_state = (uint32_t)nrf_radio_state_get(NRF_RADIO),
		.tx_queued = tx_fifo.count,
		.retries = retransmits_remaining,
		.irq_flags = interrupt_flags,
		.radio_events =
			(nrf_radio_event_check(NRF_RADIO, NRF_RADIO_EVENT_READY) ? BIT(0) : 0U) |
			(nrf_radio_event_check(NRF_RADIO, NRF_RADIO_EVENT_ADDRESS) ? BIT(1) : 0U) |
			(nrf_radio_event_check(NRF_RADIO, ESB_RADIO_EVENT_END) ? BIT(2) : 0U) |
			(nrf_radio_event_check(NRF_RADIO, NRF_RADIO_EVENT_DISABLED) ? BIT(3) : 0U),
		.timer_events =
			(nrf_timer_event_check(esb_timer.p_reg, NRF_TIMER_EVENT_COMPARE0) ? BIT(0) : 0U) |
			(nrf_timer_event_check(esb_timer.p_reg, NRF_TIMER_EVENT_COMPARE1) ? BIT(1) : 0U) |
			(nrf_timer_event_check(esb_timer.p_reg, NRF_TIMER_EVENT_COMPARE2) ? BIT(2) : 0U),
		.timer_shorts = esb_timer.p_reg->SHORTS,
		.radio_irq =
			(NVIC_GetEnableIRQ(ESB_RADIO_IRQ_NUMBER) ? BIT(0) : 0U) |
			(NVIC_GetPendingIRQ(ESB_RADIO_IRQ_NUMBER) ? BIT(1) : 0U),
		.timer_irq =
...
#endif /* CONFIG_TOTEM_ESB_DIAGNOSTICS */
...
/*
 * Application-side ACK lifetime/fairness support. Keep the Nordic ACK state
 * machine intact; only inspect or cancel one PRX pipe under the same IRQ lock.
 */
int esb_get_ack_payload_count(uint8_t pipe)
{
	if (!esb_initialized) {
		return -EACCES;
	}
	if (esb_cfg.mode != ESB_MODE_PRX || pipe >= CONFIG_ESB_PIPE_COUNT) {
		return -EINVAL;
	}

	unsigned int key = irq_lock();
	int count = 0;
	for (struct payload_wrap *entry = ack_pl_wrap_pipe[pipe];
	     entry != NULL; entry = entry->p_next) {
		count++;
	}
	irq_unlock(key);
	return count;
...
int esb_flush_ack_payloads(uint8_t pipe)
{
	if (!esb_initialized) {
		return -EACCES;
	}
	...
	unsigned int key = irq_lock();
	/* A staged ACK may already be on air. Let its transaction finish first. */
	if (esb_state == ESB_STATE_PRX_SEND_ACK && ack_pl_wrap_pipe[pipe] != NULL &&
	    nrf_radio_txaddress_get(NRF_RADIO) == pipe) {
		irq_unlock(key);
		return -EBUSY;
	}

	struct payload_wrap *entry = ack_pl_wrap_pipe[pipe];
	ack_pl_wrap_pipe[pipe] = NULL;
	while (entry != NULL) {
		struct payload_wrap *next = entry->p_next;
		if (current_payload == entry->p_payload) {
	...
	/* Otherwise the next newly queued ACK is mistaken for a completed one. */
	rx_pipe_info[pipe].ack_payload = false;
	irq_unlock(key);
	return 0;
}
