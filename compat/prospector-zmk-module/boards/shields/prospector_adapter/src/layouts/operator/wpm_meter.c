 ...
 * Totem: initialize animation work before use and run it on the display queue.
 */
...
int zmk_widget_wpm_meter_init(struct zmk_widget_wpm_meter *widget, lv_obj_t *parent) {
    ...
    /* The listener init invokes its update callback immediately. Initialize
     * its animation work before that callback can schedule the first frame. */
