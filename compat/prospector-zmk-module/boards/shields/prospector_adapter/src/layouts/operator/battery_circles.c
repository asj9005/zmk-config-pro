 ...
 *
 * Derived from prospector-zmk-module ed98221f3b52b7066dbb10ba3af8a29150b93a5a.
 * Totem: retain all peripheral state across coalesced display updates.
 */
...
/* A work item can coalesce events from both halves. Transfer the complete
...
static struct battery_circles_state battery_circles_get_state(const zmk_event_t *eh) {
    /* ZMK_DISPLAY_WIDGET_LISTENER serializes this getter with its state mutex.
    ...
    if (conn_ev != NULL && conn_ev->slot < PERIPHERAL_COUNT) {
        state.connected[conn_ev->slot] = conn_ev->connected;
    }
    return state;
}
