...
/* Directly owned modifiers publish a state notification after the HID change.
 * Reuse the normal display queue; this event never mutates keyboard state. */
ZMK_SUBSCRIPTION(widget_modifier_indicator, zmk_modifiers_state_changed);
