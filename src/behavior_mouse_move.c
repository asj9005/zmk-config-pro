...
/* Based on pinned ZMK 904c9aec8822d79149d42c8a9a77e8828eb08f5a.
 * The cursor uses a late acceleration boost; scrolling keeps upstream behavior.
 */
#define DT_DRV_COMPAT totem_behavior_mouse_move
...
static float speed(const struct behavior_input_two_axis_config *config, uint16_t code,
    ...
    uint32_t duration_ms = (uint32_t)MIN(
        1000 * duration_ticks / CONFIG_SYS_CLOCK_TICKS_PER_SEC, UINT32_MAX);
    return totem_pointing_curve_speed(
        max_speed, duration_ms, config->time_to_max_speed_ms, accel_exp,
        config->acceleration_boost_start_ms, config->acceleration_boost_numerator,
        config->acceleration_boost_denominator);
}
...
static int mouse_move_adjust_speed(const struct device *dev, int16_t dx, int16_t dy) {
    struct behavior_input_two_axis_data *data = dev->data;
...
static int on_keymap_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    mouse_move_adjust_speed(behavior_dev, x, y);
    return 0;
}
