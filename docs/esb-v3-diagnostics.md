...
`ci_only_totem_left_esb_v3_startdelay_testkey.uf2`는 일반 왼쪽 diagnostic에 `CONFIG_TOTEM_ESB_DIAGNOSTIC_START_DELAY_MS=3000`만 추가한다. 같은 4096바이트 초기화 스레드에서 암호화·무선 초기화 전에 한 번만 3초 기다린다. USB 호스트나 DTR을 기다리지 않고 `UART_LINE_CTRL`도 켜지 않으며, 초기화 단계별 지연도 추가하지 않는다. 기본값 0인 일반 빌드는 즉시 시작한다.
...
이 비교를 준비한 실물에서는 `35744b6` 자동 시작 왼쪽의 정상 입력 후 끊김과 리셋 뒤 USB 설명자 오류가 보고됐다. 부트로더 USB는 동작했고 `CURRENT.UF2`의 애플리케이션 107,264바이트는 제공한 자동 시작 UF2와 전부 일치했다. 파일 오업로드는 배제했지만, 지연 비교 빌드의 실기기 안정성은 아직 확인하지 않았다.
